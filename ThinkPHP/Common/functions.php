...
function T($template = '', $layer = ''){
    ...
    if('' == $file){
        ...
        $file = substr_count($file, '/') > 1 ? substr_replace($file, $depr, strrpos($file, '/'), 1)
            : str_replace('/', $depr, $file);
    }
...
function U($url = '', $vars = '', $suffix = true, $domain = false){
    ...
    if(C('URL_MODEL') == 0){ // 普通模式URL转换
        ...
        if(isset($route)){
            ...
            $url    = __APP__ . '/' . ($module ? $module . MODULE_PATHINFO_DEPR
                    : '') . implode($depr, array_reverse($var));
        }
        if($urlCase){
            $url = strtolower($url);
        }
...
function session($name = '', $value = ''){
    ...
    if(is_array($name)){ // session初始化 在session_start 之前调用
        ...
        if(C('SESSION_TYPE')){ // 读取session驱动
            ...
            $hander = new $class();
            session_set_save_handler(array(&$hander, "open"), array(&$hander, "close"), array(
                    &$hander,
                    "read"
                ), array(&$hander, "write"), array(&$hander, "destroy"), array(&$hander, "gc"));
        }
        // 启动session
