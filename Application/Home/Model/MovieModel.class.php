...
class MovieModel extends Model{
    ...
    public function playReplace($purl, $pid, $type){
        ...
        foreach($purl as $key => $value){
            ...
            if($type){
                $payurl[$key]["url"] = url_change("player/down", array(
                    "name" => "down",
                    "pid"  => $pid,
                    "n"    => $key + 1
                ));
            }else{
                $payurl[$key]["url"] = url_change("player/index", array(
                    "name" => "player",
                    "pid"  => $pid,
                    "n"    => $key + 1
                ));
    ...
    public function getPlayer($id, $pid, $n){
        ...
        $player['player_code'] = '<iframe scrolling="no" frameborder="0" src="<$url>" style="height:100%; width:100%;" allowfullscreen></iframe>';
        //$src = U('Home/Movie/getPlayers/');
        //$player['player_code'] = '<iframe scrolling="no" frameborder="0" src="'.$src.'?url=<$url>" style="height:100%; width:100%;" allowfullscreen></iframe>';
        $player["url"] = $play[1];
        ...
        //开始爬去只有播放器没有其他杂志
        //$url__ = $player["url"];
        //$moviess = file_get_contents($url__);
        //$pattern = '/(.*?)\.swf|\.flv/ism';
        //preg_match_all($pattern, $moviess, $result);
        //p($moviess);

        $player["player_code"] = str_replace(array(
            '<$url>',
            '<$lasturl>',
            '<$nexturl>',
            '<$lastplay>',
            ...
            $lastplay,
            $nextplay
        ), htmlspecialchars_decode($player["player_code"]));
        unset($player['sort']);
